...
class MMKRLNegativeSampling(Strategy):

    def __init__(
        self, model=None, loss=None, batch_size=256, regul_rate=0.0, l3_regul_rate=0.0
    ):
        super(MMKRLNegativeSampling, self).__init__()
        self.model = model
        self.loss = loss
    ...
    def forward(self, data, fast_return=False):
        score, ka_loss = self.model(data, mse=True)
        p_score = self._get_positive_score(score)
        if fast_return:
            return p_score
        n_score = self._get_negative_score(score)
        loss_res = self.loss(p_score, n_score) + ka_loss
